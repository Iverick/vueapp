...
local_deps() {
  ...
  add_ppa
  sudo "$debconf_fix" apt-fast install -f -y libargon2-dev libmagickwand-dev libpq-dev libfreetype6-dev libicu-dev libjpeg-dev libpng-dev libonig-dev libxslt1-dev libaspell-dev libcurl4-gnutls-dev libenchant-dev libc-client2007e-dev libkrb5-dev libldap-dev libsodium-dev libtidy-dev libwebp-dev libxpm-dev libzip-dev
}
...
install() {
  ...
  sudo mkdir -m 777 -p /usr/local/php
  wait "$to_wait"
...
configure() {
  ...
  sudo mkdir -p /lib/systemd/system /etc/apache2/mods-available /etc/apache2/conf-available
  sudo cp -f "$install_dir"/etc/init.d/php-fpm /etc/init.d/php"$version"-fpm
  ...
  sudo ln -f "$install_dir"/include/php /usr/include/php/"$(php-config --extension-dir | grep -Eo -m 1 "[0-9]{8}")"
